...
if($#ARGV != 5){
	print "use: parc_prepro.pl <file> <score> <text-output> <hypo-output> <senna-path> <tree_tagger-path>\n";
	exit 0;
}

my($file, $score, $t_file, $h_file, $senna_path, $treetagger_path, $max) = (shift, shift, shift, shift, shift, shift, shift);
